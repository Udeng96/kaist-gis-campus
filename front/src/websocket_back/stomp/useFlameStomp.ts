...
export const useFlameStomp = (url: string, subscribeMappings: { topic: string, handler: (message: any) => void }[]) => {
            ...
            debug: (msg) => {
                if (msg.includes("Connection closed")) {
                    if (connectionStatus.current === "connected") {
                        ...
                        console.log(`[SOCKET-FLAME] 연결 끊김`);
                    }
                    return; // 이후 핑퐁 등 무시
                }
                if (msg.includes("Web Socket Opened") ||
                    ...
                    if (connectionStatus.current === "disconnected") {
                        ...
                        console.log(`[SOCKET-FLAME] 연결 성공`);
                    }
                    return;
                }
            },
